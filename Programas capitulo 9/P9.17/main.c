...
void cambia(FILE *ap1, FILE *ap2)
{
   ...
   while (!feof(ap1))
   {
       ...
       while (cad2!=NULL)
       {
           ...
           i = strlen(cad1);
           j = strlen(cad2);
           k = i - j;    /* En kkse almacena la diferencia de las longitudes de las cadenas cad1 y cad2. */
           if (k)
           {
               strncpy(aux, cad1, k);
               /* Se copia la subcadena de kkcaracteres de cad1 a aux—desde el
       ...
       fputs(cad1, ap2);      /* Se ecribe cadena correcta en el archivo ap2. */
   }
}
